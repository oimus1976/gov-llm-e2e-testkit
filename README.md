...
本ツールを利用するにあたり、あらかじめ次の点をご確認ください。

### 動作環境について

本ツールは、以下の環境を前提として設計されています。
...
次のような環境では、本ツールをそのまま実行できない場合があります。
...
条例データ（例規 HTML）は、本ツールには含まれていません。

- DVD、USB メモリ、ネットワークドライブ等に存在する
  既存の条例データを、そのまま指定して利用します
- 条例データを本ツールのフォルダ内にコピーする必要はありません
...
### 初期セットアップ・実行前準備（新規端末向け）

本リポジトリを **新しい端末でクローンした直後**に必要となる初期セットアップ  
（Python 依存関係、Playwright の準備、`.env` ファイルの扱い）や、  
ユーザーテストを開始するまでの **実行前準備手順**については、以下のドキュメントを参照してください。

- `docs/operation/Setup_and_UserTest_Guide.md`

※ 本ドキュメントは **実行前準備・環境構築の正本**です。  
※ 実際の業務手順・禁止事項・成果物の扱いについては、  
　下記の **F8 実務手順を正本**として参照してください。

了解。
**README に追記する最小・安全・衝突しない文案**を出します。
既存の *「README は概要、正本は別」* という思想を**崩さない配置**です。

---

### 初期セットアップ（新規端末・最初に一度だけ）

新しい端末で本リポジトリをクローンした場合、`.env` が存在しないとテストは実行できません。
...
- `.env` が既に存在する場合は **何も行いません**（冪等）
- `.env` が無い場合は、既知のテンプレートから自動生成します
- ダミー値や未設定項目がある場合は、スクリプトが編集を促します

⚠️ 注意
この手順は **実行前準備（環境構築）専用**です。
実際の業務手順・禁止事項・成果物の扱いについては、
下記の **実務手順ドキュメントを正本**として参照してください。
