...
def wait_for_answer_text(
    *,
    ...
    """
    - probe を利用し、submit_id に対応する完了済み回答テキスト（raw str）を返す
    - v0.1 では証跡保存は行わない
    - page は probe v0.2（run_graphql_probe）が response を監視するために必要な観測コンテキスト
    - answer_probe 自身は page に対して操作を行わない（DOM/UI 非依存）
    """
```

#### Parameters

- `page`（必須）: probe v0.2 が response hook により通信を観測し、chat 境界を内部的に確定するためのコンテキスト
...
#### probe 呼び出しのマッピング（v0.1）

v0.1 は scripts.probe_v0_2.run_graphql_probe を再利用し、再実装しない。
answer_probe は probe の完了意味論に介入せず、戻り値（summary）を解釈するのみとする。
...
class _ProbeClient:
    def wait_for_answer_text(
        ...
        *,
        ...
        """
        - v0.1 では submit_id / chat_id は probe 呼び出しに使用しない
        - chat 境界は probe が通信観測から内部的に確定する
        - capture_seconds は timeout_sec をそのままマッピングする
        """
...
- 本設計が禁止するのは「answer_probe が証跡保存機構を提供すること」であり、probe 実装内部の出力（jsonl/summary）までを否定するものではない（v0.1 では制御対象外）。
