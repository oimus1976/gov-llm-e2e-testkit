# 📘 PROJECT_STATUS v0.7.21

**— F9-D 完了／F9-B 仕様 FIX／F9-A（質問セット・条例バインディング）仕様確定反映版 —**
...
#### **F9-A：質問セット・条例バインディング（仕様確定／実装成立）【更新】**
...
- 質問テンプレは **抽象表現のみを許容**（この条例／第○条／第○条第○項）
...
- **A-2**：条例IDと質問テンプレ集合の束縛
- **A-3**：条・項の自動解決（構造的一意性のみ）
- **A-4**：自動解決不能時の人間入力による最終解決（I/O 仕様 FIX）
...
## 2. 設計・構造面での更新（v0.7.21）

- **Spec_F9-A 系仕様を v0.1r として確定**
  - 質問テンプレ許容表現・禁止事項を明文化
  - CSV 起点の運用モデルを正式採用
  - 人間入力による解決（A-4）の I/O を FIX
- **F9-A の実装・テスト成立を STATUS に反映**
