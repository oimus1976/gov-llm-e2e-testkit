...
- pytest の責務を「契約・成立・非例外確認」に限定する工程5を完了
- F4 writer 系テストから内容評価（Markdown / execution_context の assert）を除去
- RAG basic / advanced v0.1 テストから意味評価・品質評価（keyword 判定 / PASS-FAIL）を除去
- legacy ask ベースの pytest テストを削除し、pytest 正系から完全除外

### Fixed

- pytest が評価主体として振る舞っていた実装を是正
- 新端末・CI 環境差による pytest 不安定要因を解消

### Notes

- 本バージョンは機能変更を伴わない
- テスト計画再構築計画 Backlog の **工程5（実装修正・反映）完了**を記録するものである
...
- 全 pytest テストに対する A/B/C 判定完了の事実を PROJECT_STATUS に反映
- 工程4（設計と実装の差分確定）完了を明記
- 全 test file（工程5.1–5.18）の A/B/C 判定確定
  - 「直さない判断」を含む設計根拠付きレビューの完了
- 差分レビュー成果物 `impl_diff_review_v0.2.0.md` を正式成果物として位置づけ

### Changed

- テストプラン再構築計画に関する用語を整理
  - Roadmap 上の進捗単位を「フェーズ（F1–F10）」に限定
  - テストプラン再構築は「工程（工程0–工程5）」として分離
- PROJECT_STATUS 全文を上記用語整理に基づき差し替え
- 工程5（実装修正）以降を PROJECT_STATUS から切り離し、
  独立ドキュメントで管理する方針を明文化

### Notes

- 本バージョンではコード修正は行っていない
- 変更内容は判断・記録・フェーズ遷移の明文化のみ
...
- 新端末向けの冪等な初期化スクリプト `scripts/bootstrap_env.py` を追加
- `.env` 不在時でも安全にローカル実行を開始できる初期化手順を整備

### Changed

- README に初期セットアップ（bootstrap 実行）の案内を追記

### Notes

- 本変更は F9 成果物・評価方針・I/F に影響を与えない
- F10-A 着手前提条件としての運用基盤整備
...
- F9-A Question Resolution を、README / FAQ / 実装の三点で最終整合
- F9-A を「非対話・固定配置・一般ユーザー運用成立」として定義確定
- README をプロジェクト入口ドキュメントとして位置づけ
- FAQ を README と完全整合する内容に更新

### Clarified

- F9-A における user_input は「実行時入力」ではなく
  結果データ上の未確定状態表現であることを明文化
- run/ ディレクトリの責務を「実行専用（編集禁止）」として明示
- PowerShell 版 / bat 版の役割差を仕様として整理
...
- README.md を再構成し、想定読者（一般利用者／管理者・開発者）と参照すべき正本文書を明確化
- 一般利用者（職員・評価者）向けの実行手引書として  
  `docs/README_Question_Resolution.md` を正式に位置づけ
- README.md から Question Resolution 手引書への公式導線を追加

### F9-A

- F9-A（質問セット・条例バインディング）について、  
  **ユーザーが実際に実行・確認できる運用形を前提とする位置づけ**を明確化
- pytest による内部検証に加え、  
  ユーザー実行を想定した確認フローをドキュメント体系に反映

### Project Management

- ドキュメント体系を「入口（README）／手引書／仕様正本」に分離し、
  今後の運用・引き継ぎ時の混乱を防止
...
- Spec_F9-A_Implementation_v0.1
  - F9-A における処理フローを実装拘束レベルで定義
  - 条・項を必ず解決する制御フローを明文化
  - 自動探索とユーザー入力を組み合わせた解決保証ポリシーを確定
...
- 質問セット入力形式を CSV に一本化し、YAML 記述例を削除
- 抽象表現の定義を実装仕様（第○条第○項）に統一
- Spec_F9-A_Implementation における手動テスト対象表記を修正（CSV 明示）

### Notes

- 設計思想・責務分離に変更はない
- 実装およびテスト結果（A-0〜A-3 green）との整合を目的とした改訂
...
- Spec_F9-A_Question_Set_and_Binding_v0.1
  - 質問セットと条例バインディングの分離仕様を定義
  - 抽象質問（この条例／第○条／第○項）の解決モデルを明文化
  - 条・項を必ず解決して後続フェーズへ渡す解決保証ポリシーを追加

### Changed

- F9 Backlog A の位置づけを
  「質問文と条例参照の構造確定フェーズ」として明確化
...
- PROJECT_STATUS を更新し、
  F9-B を「仕様・実装 完了」として確定

### Notes

- 仕様変更はなし（Spec_F9-B v0.1r に準拠）
...
- Spec_F9-B_Execution_Conditions_Freeze_v0.1r を FIX
  - ナレッジ切替をアカウント差分で行う実行モデルを確定
  - 再投入禁止原則を正式仕様として明文化
  - `.env` 切替機構を F9 の正式実装対象として確定
  - 比較不能条件を「実行条件の記録不能」として定義

### Changed

- PROJECT_STATUS を v0.7.17 に更新
  - F9-B を仕様 FIX として反映
  - F9 の成果範囲を「正式実装を含む」形に明確化

### Notes

- 本更新は仕様確定であり、実装は次工程で行う
- F4 の実証結果を F9 の設計判断根拠として正式採用
...
- Schema_dataset_v0.2 を FIX として確定
  - 必須 / 任意 / 禁止フィールドを明文化
- dataset.yaml v0.2 を実体として生成（tc09-dataset）
- verify-diff により f8_runs → datasets の answer.md が
  **全件完全一致することを実証**
- entries が空の場合に人間向けエラーで停止する生成側ガードを追加

### Changed

- F9-D（下流整合）を **完了フェーズ**として PROJECT_STATUS に反映
- dataset を「意味・評価・実行条件を持たない束ね単位」として最終確定

### Verified

- pytest 非依存・人間実行前提で  
  dataset 構築とコピー完全性検証が成立することを確認

### Notes

- 本バージョンは **設計・実装・実体・検証の一致を宣言する版**
- dataset / schema の責務定義に変更はなく、  
  **運用成立を反映する状態更新**である
